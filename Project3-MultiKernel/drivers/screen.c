...
void screen_write_ch(char ch)
{
    ...
    else if (ch == 8 || ch == 127)
    {
        ...
        new_screen[((*current_running)->cursor_y) * SCREEN_WIDTH + ((*current_running)->cursor_x)] = ' ';
    }
    else
    {
        new_screen[((*current_running)->cursor_y) * SCREEN_WIDTH + ((*current_running)->cursor_x)] = ch;
        (*current_running)->cursor_x++;
    }
...
void screen_reflush(void)
{
    ...
    int origin_x = (*current_running)->cursor_x;
    int origin_y = (*current_running)->cursor_y;
    ...
    (*current_running)->cursor_x = origin_x;
    (*current_running)->cursor_y = origin_y;
}