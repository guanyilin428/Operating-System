...
{
    return invoke_syscall(SYSCALL_MUTEX_GET, handle, IGNORE, IGNORE, IGNORE);
}

void mutex_block(int id)
{
    invoke_syscall(SYSCALL_MUTEX_LOCK, id, IGNORE, IGNORE, IGNORE);
}

void mutex_unlock(int id)
{
    invoke_syscall(SYSCALL_MUTEX_UNLOCK, id, IGNORE, IGNORE, IGNORE);
}
...
int mthread_create(mthread_t *thread,
                   ...
                   void *arg)
{
    invoke_syscall(SYSCALL_MTHREAD_CREATE, thread, start_routine, arg, IGNORE);
}
