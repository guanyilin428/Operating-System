...
static inline uintptr_t load_elf(
    unsigned char elf_binary[], unsigned length, uintptr_t pgdir, pcb_t* pcb,
    uintptr_t (*prepare_page_for_va)(uintptr_t va, uintptr_t pgdir, pcb_t* pcb))
{
                ...
                if (i < phdr->p_filesz) {
                    unsigned char *bytes_of_page =
                        (unsigned char *)prepare_page_for_va(
                            (uintptr_t)(phdr->p_vaddr + i), pgdir, pcb);
                    memcpy(
                        bytes_of_page,
