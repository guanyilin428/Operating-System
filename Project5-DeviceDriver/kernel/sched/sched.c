...
int do_fork()
{
    ...
    new->user_sp = allocPage(new) + PAGE_SIZE;
    new->kernel_sp = allocPage(new) + PAGE_SIZE;
...
void do_scheduler(void)
{
    ...
    if(list_empty(&ready_queue)){
        if(prev != &pid0_pcb_core0 && prev != &pid0_pcb_core1 && prev->status == TASK_RUNNING)
            return;
        *current_running = get_current_cpu_id() ? &pid0_pcb_core1 : &pid0_pcb_core0;
    }
    else{
        if(prev != &pid0_pcb_core0 && prev != &pid0_pcb_core1 && prev->status == TASK_RUNNING){
    ...
    unsigned long ppn = kva2pa((*current_running)->pgdir)>>NORMAL_PAGE_SHIFT;
...
void do_process_show()
{
    ...
    for(i = 1; i < NUM_MAX_TASK; i++){
        ...
        {
        case TASK_RUNNING:
        prints("PID:%d STATUS:RUNNING\n\r", i);
        break;

        case TASK_READY:
        prints("PID:%d STATUS:READY\n\r", i);
        break;

        case TASK_BLOCKED:
        prints("PID:%d STATUS:BLOCKED\n\r", i);
        break;

        case TASK_ZOMBIE:
        prints("PID:%d STATUS:ZOMBIE\n\r", i);
        break;
...
// release the sources of corresponding pcb
